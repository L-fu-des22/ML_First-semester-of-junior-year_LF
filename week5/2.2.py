...
def readfile():
    array_of_img = []
    directory_name_1 = 'E:\\Junior first semester\\374-Machine Learning System Design\\week5\\homework52\\data\\Cartoon'
    i = 0
...
class Digit_0(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(51529, 7)

    def forward(self, x):
        x = x.view(-1, 51529)  # 拉平，-1自动计算维度，
        x = self.drop1(x)
...
if __name__ == '__main__':
    ...
    Test_acc_maxx_index = np.array(Test_acc_maxx_index)[0]
    ...
    Test_acc_minn_index = np.array(Test_acc_minn_index)[0]
    ...
    # 表现最好的loss
    l_train_maxx = plt.plot(x, Train_loss_maxx.flatten().reshape((len(x), 1)), 'ro-',
                            label=f'train_best:Fold{Test_acc_maxx_index}')
    l_test_maxx = plt.plot(x, Test_loss_maxx.flatten().reshape((len(x), 1)), 'r^-',
                           label=f'test_best:Fold{Test_acc_maxx_index}')

    # 表现最差的loss
    l_train_minn = plt.plot(x, Train_loss_minn.flatten().reshape((len(x), 1)), 'go-',
                            label=f'train_worst:Fold{Test_acc_minn_index}')
    l_test_minn = plt.plot(x, Test_loss_minn.flatten().reshape((len(x), 1)), 'g^-',
                           label=f'test_worst:Fold{Test_acc_minn_index}')
    ...
    y_axis_max = Train_loss_minn.max()

    ax.annotate(f'layer={hidden_num_label}', (10, y_axis_max), fontsize="x-small", annotation_clip=False)
    ax.annotate(f'lambda={lamb_label}', (10, y_axis_max * 0.95), fontsize="x-small", annotation_clip=False)
    ax.annotate(f'acc_train_convege_best={acc_train_convege_best}%', (10, y_axis_max * 0.9), fontsize="x-small",
                annotation_clip=False)
    ax.annotate(f'acc_test_convege_best={acc_test_convege_best}%', (10, y_axis_max * 0.85), fontsize="x-small",
                annotation_clip=False)
    ax.annotate(f'acc_train_convege_worst={acc_train_convege_worst}%', (10, y_axis_max * 0.8), fontsize="x-small",
                annotation_clip=False)
    ax.annotate(f'acc_test_convege_worst={acc_test_convege_worst}%', (10, y_axis_max * 0.75), fontsize="x-small",
                annotation_clip=False)
    ax.annotate(f'learning_rate={lr}%', (10, y_axis_max * 0.7), fontsize="x-small",
                annotation_clip=False)
